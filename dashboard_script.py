...
language_counts = pd.DataFrame(data['language'].value_counts()).reset_index().rename(columns={'index': 'language', 'language': 'language_count'})
top_10_languages = language_counts.head(10).sort_values(by='language_count', ascending=True)  
plt.figure(figsize=(10, 8)) 
plt.barh(top_10_languages['language'], top_10_languages['language_count'], color = '#20b0cf', edgecolor = '#0e4551')  
...
plt.xlabel('forks_count', fontsize=20)
st.pyplot(plt)
...
# 9. Box and Whiskers graph for contributers count for each language
st.write("### 9. Box and Whiskers graph for pull requests count for each language")
sns.set_style("whitegrid")
contributors_box = sns.catplot(data=data, kind='box', y='language', x='contributors', height=20, palette='rocket', sym='')
plt.yticks(fontsize=16)
plt.xticks(fontsize=16)
plt.ylabel('language', fontsize=20, )
